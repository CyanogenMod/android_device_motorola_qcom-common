...
for FILE in `egrep -v '(^#|^$)' ../$DEVICE/device-proprietary-files.txt`; do
  COUNT=`expr $COUNT - 1`
  if [ $COUNT = "0" ] && [ -f ../qcom-common/proprietary-files.txt ]; then
    LINEEND=" \\"
  elif [ $COUNT = "0" ]; then
  LINEEND=""
  fi
  ...
  if [ -z "$DEST" ]; then
    ...
    if [ $DEST != lib/libtime_genoff.so ]; then
      if [[ $DEST != app/* ]]; then
...
\$(call inherit-product, vendor/$VENDOR/$FAMILY/$FAMILY-vendor.mk)
...
LINEEND=" \\"
COUNT=`wc -l ../$FAMILY/common-proprietary-files.txt | awk {'print $1'}`
DISM=`egrep -c '(^#|^$)' ../$FAMILY/common-proprietary-files.txt`
COUNT=`expr $COUNT - $DISM`
...
echo "ifeq (\$(TARGET_BOARD_PLATFORM),msm8960)" >> ../../../${OUTDIR}/proprietary/app/Android.mk
echo ""  >> ../../../${OUTDIR}/proprietary/app/Android.mk
...
echo "ifeq (\$(TARGET_BOARD_PLATFORM),msm8960)" >> ../../../${OUTDIR}/proprietary/framework/Android.mk
echo ""  >> ../../../${OUTDIR}/proprietary/framework/Android.mk
...
LINEEND=" \\"
COUNT=`wc -l ../qcom-common/common-proprietary-files.txt | awk {'print $1'}`
DISM=`egrep -c '(^#|^$)' ../qcom-common/common-proprietary-files.txt`
COUNT=`expr $COUNT - $DISM`
